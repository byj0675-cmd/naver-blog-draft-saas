...
import { BrandBrief, emptyBrandBrief } from "@shared/brandBrief";
...
function Topbar({ section, setOpen, onAddBrand }: { section: Section; setOpen: (b: boolean) => void; onAddBrand: () => void }) {
  const label = nav.find(n => n.id === section)?.label;
  return <header className="flex h-[76px] items-center justify-between border-b border-[#e7e4de] bg-[#fbfaf8] px-5 lg:px-10"><div className="flex items-center gap-3"><button className="lg:hidden" onClick={() => setOpen(true)}><Menu size={21}/></button><div><p className="text-[11px] font-semibold text-[#a0a3a5]">작업 공간 / {label}</p><h1 className="mt-0.5 text-[18px] font-extrabold tracking-[-.04em]">{label}</h1></div></div><div className="flex items-center gap-2.5"><button className="hidden rounded-lg border border-[#e7e4de] bg-white p-2.5 text-[#858b91] sm:block"><Search size={16}/></button><button className="relative rounded-lg border border-[#e7e4de] bg-white p-2.5 text-[#858b91]"><Bell size={16}/><i className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-[#e86550]"/></button><Button onClick={() => startKakaoLogin()} variant="outline" className="hidden h-9 rounded-lg border-[#e7e4de] bg-white px-3 text-xs font-bold text-[#606a73] sm:inline-flex">카카오 로그인</Button><Button onClick={onAddBrand} className="ml-1 h-9 gap-1.5 rounded-lg bg-[#e86550] px-3.5 text-xs font-bold text-white hover:bg-[#d95744]"><Plus size={15}/> 브랜드 추가</Button></div></header>;
}
...
function Generate() {
  ...
  const [brandId, setBrandId] = useState<number | null>(null); const selectedBrandId = brandId ?? brandList.data?.[0]?.id ?? 0; const selectedBrand = brandList.data?.find(brand => brand.id === selectedBrandId);
  ...
  const generateDraft = trpc.content.generate.useMutation({
    onSuccess: result => {
      ...
      if (Array.isArray((result.draft as { titleCandidates?: string[] }).titleCandidates)) setTitleCandidates((result.draft as { titleCandidates: string[] }).titleCandidates);
      setGenerated(true);
...
function BrandCreateDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  const utils = trpc.useUtils();
  const createBrand = trpc.brands.create.useMutation({
    onSuccess: async () => { localStorage.removeItem("blogmate.onboarding.draft"); await utils.brands.list.invalidate(); toast.success("업체 정보가 저장됐어요."); onClose(); },
    onError: error => toast.error(error.message || "업체 정보 저장에 실패했습니다."),
  });
  const steps = ["기본 정보", "서비스", "전문성", "고객 질문", "글쓰기 기준", "첫 글 확인"];
  const [step, setStep] = useState(0);
  const [saveStatus, setSaveStatus] = useState("입력 전");
  const [form, setForm] = useState(() => { try { const saved = localStorage.getItem("blogmate.onboarding.draft"); return saved ? JSON.parse(saved) : { name: "", industry: "", services: "", audience: "", strengths: "", firstTopic: "", brief: { ...emptyBrandBrief } }; } catch { return { name: "", industry: "", services: "", audience: "", strengths: "", firstTopic: "", brief: { ...emptyBrandBrief } }; } });
  useEffect(() => { if (!open) return; const timer = window.setTimeout(() => { localStorage.setItem("blogmate.onboarding.draft", JSON.stringify(form)); setSaveStatus("자동 저장됨"); }, 250); return () => window.clearTimeout(timer); }, [form, open]);
  const update = (key: "name" | "industry" | "services" | "audience" | "strengths" | "firstTopic", value: string) => setForm((prev: typeof form) => ({ ...prev, [key]: value }));
  const updateBrief = (key: keyof BrandBrief, value: string) => setForm((prev: typeof form) => ({ ...prev, brief: { ...prev.brief, [key]: value } }));
  const close = () => { if (!createBrand.isPending) { setStep(0); onClose(); } };
  const next = () => { if (step === 0 && (!form.name.trim() || !form.industry.trim())) { toast.error("상호명과 업종을 입력해 주세요."); return; } setStep(prev => Math.min(5, prev + 1)); };
  const submit = (event: React.FormEvent) => { event.preventDefault(); if (!form.name.trim() || !form.industry.trim()) { setStep(0); toast.error("상호명과 업종을 입력해 주세요."); return; } createBrand.mutate({ name: form.name, industry: form.industry, services: form.services, audience: form.audience, strengths: form.strengths, brief: form.brief }); };
  if (!open) return null;
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#18202b]/45 p-0 backdrop-blur-[2px] sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="onboarding-title"><div className="flex max-h-[95vh] w-full max-w-[760px] flex-col overflow-hidden rounded-t-[24px] bg-[#fbfaf8] shadow-[0_24px_80px_rgba(24,32,43,.24)] sm:max-h-[90vh] sm:rounded-[24px]"><div className="border-b border-[#e9e5df] bg-white px-5 pb-4 pt-5 sm:px-8 sm:pt-7"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#e86550]">업체 등록 · {step + 1}/{steps.length}</p><h2 id="onboarding-title" className="mt-2 text-xl font-black tracking-[-.04em] text-[#18202b]">우리 업체를 먼저 알려주세요.</h2><p className="mt-1.5 text-xs leading-5 text-[#7b838b]">매번 프롬프트를 쓰지 않아도 업체에 맞는 초안을 만들 수 있어요.</p></div><div className="flex items-center gap-2"><span className="text-[10px] font-bold text-[#9da3a6]" aria-live="polite">{saveStatus}</span><button type="button" onClick={close} className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#f4f1ec] text-[#69737b] transition hover:bg-[#ece7e0]" aria-label="업체 등록 닫기"><X size={17}/></button></div></div><div className="mt-5 grid grid-cols-7 gap-1.5" aria-label="업체 등록 진행 단계">{steps.map((item, index) => <button type="button" key={item} onClick={() => index < step && setStep(index)} className="group text-left" aria-current={index === step ? "step" : undefined}><div className={"h-1.5 rounded-full transition " + (index <= step ? "bg-[#e86550]" : "bg-[#e9e5df]")}/><span className={"mt-1.5 hidden truncate text-[10px] font-bold sm:block " + (index === step ? "text-[#e86550]" : "text-[#a0a5a7]")}>{item}</span></button>)}</div><p className="mt-2 text-[10px] font-bold text-[#a0a5a7] sm:hidden">{steps[step]}</p></div>
  <form onSubmit={submit} className="min-h-0 flex-1 overflow-y-auto px-5 py-6 sm:px-8 sm:py-7">
    {step === 0 && <section className="space-y-5"><div><h3 className="text-lg font-black tracking-[-.03em] text-[#18202b]">고객이 검색할 때 보이는 정보</h3><p className="mt-1.5 text-xs leading-5 text-[#7b838b]">상호명과 업종만 먼저 입력해도 시작할 수 있습니다.</p></div><div className="grid gap-4 sm:grid-cols-2"><div><label htmlFor="onboarding-name" className={labelClass}>상호명 <span className="text-[#e86550]">*</span></label><input id="onboarding-name" autoFocus value={form.name} onChange={event => update("name", event.target.value)} placeholder="예: 바른움직임 필라테스" className={inputClass}/></div><div><label htmlFor="onboarding-industry" className={labelClass}>업종 <span className="text-[#e86550]">*</span></label><input id="onboarding-industry" value={form.industry} onChange={event => update("industry", event.target.value)} placeholder="예: 필라테스 스튜디오" className={inputClass}/></div><div>{field("location", "주요 지역", "예: 서울 마포구 합정동")}</div><div>{field("address", "주소", "예: 도로명 주소")}</div><div>{field("phone", "연락처", "예: 02-000-0000")}</div><div>{field("businessHours", "영업시간", "예: 평일 07:00~22:00")}</div><div className="sm:col-span-2">{field("website", "홈페이지 또는 예약 링크", "https://...")}</div></div></section>}
    {step === 1 && <section className="space-y-5"><div><h3 className="text-lg font-black tracking-[-.03em] text-[#18202b]">어떤 서비스를 제공하나요?</h3><p className="mt-1.5 text-xs leading-5 text-[#7b838b]">실제 서비스·가격·고객을 적을수록 다른 업체 정보가 섞이지 않아요.</p></div><div className="grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><label htmlFor="onboarding-services" className={labelClass}>주요 상품·서비스</label><textarea id="onboarding-services" value={form.services} onChange={event => update("services", event.target.value)} placeholder="예: 개인레슨, 그룹레슨, 체형교정 수업" className={textareaClass}/></div><div>{field("priceInfo", "가격 또는 견적 기준", "예: 개인레슨 1회 60,000원 / 현장 확인 후 견적")}</div><div>{field("uniquePoints", "고객이 선택하는 이유", "예: 원장 직접 상담, 소규모 수업", true)}</div><div><label htmlFor="onboarding-audience" className={labelClass}>주요 고객</label><input id="onboarding-audience" value={form.audience} onChange={event => update("audience", event.target.value)} placeholder="예: 운동 초보자, 직장인" className={inputClass}/></div><div><label htmlFor="onboarding-strengths" className={labelClass}>대표 강점</label><input id="onboarding-strengths" value={form.strengths} onChange={event => update("strengths", event.target.value)} placeholder="예: 예약 전 상담이 꼼꼼해요" className={inputClass}/></div></div></section>}
    {step === 2 && <section className="space-y-5"><div><h3 className="text-lg font-black tracking-[-.03em] text-[#18202b]">사장님이 가장 잘 아는 내용</h3><p className="mt-1.5 text-xs leading-5 text-[#7b838b]">전문가만 알고 있는 과정과 현장 노하우가 내 업체다운 글을 만듭니다.</p></div><div className="space-y-4">{field("brandStory", "업체를 시작한 이유 또는 브랜드 이야기", "예: 현장 경험을 바탕으로 초보자도 안심할 수 있는 서비스를 만들었습니다.", true)}{field("factsToUse", "글에 꼭 넣고 싶은 실제 사실", "예: 자격, 장비, 작업 과정, 수업 인원, 실제 운영 방식을 적어주세요.", true)}</div></section>}
    {step === 3 && <section className="space-y-5"><div><h3 className="text-lg font-black tracking-[-.03em] text-[#18202b]">고객이 자주 묻는 것을 알려주세요.</h3><p className="mt-1.5 text-xs leading-5 text-[#7b838b]">상담할 때 반복하는 답변은 글의 좋은 소재가 됩니다.</p></div><div className="space-y-4">{field("customerQuestions", "자주 묻는 질문과 공식 답변", "예: 운동을 처음 해도 되나요? 네, 첫 상담 후 현재 상태에 맞춰 진행합니다.", true)}{field("forbiddenClaims", "답변에서 피해야 할 표현", "예: 완치, 100% 효과, 무조건 최저가", true)}</div></section>}
    {step === 4 && <section className="space-y-5"><div><h3 className="text-lg font-black tracking-[-.03em] text-[#18202b]">글쓰기 기준을 정해볼까요?</h3><p className="mt-1.5 text-xs leading-5 text-[#7b838b]">저장한 기준을 바탕으로 매번 프롬프트를 작성하지 않아도 됩니다.</p></div><div className="grid gap-4 sm:grid-cols-2">{field("primaryKeywords", "핵심 키워드", "예: 합정동 필라테스, 마포구 체형교정")}{field("secondaryKeywords", "보조 키워드", "예: 필라테스 초보, 직장인 운동")}{field("toneNotes", "원하는 말투", "예: 차분한 존댓말, 과장 없이 상담하듯", true)}{field("callToAction", "글 마지막에 안내할 행동", "예: 상담 예약, 전화 문의, 프로필 링크 확인", true)}</div></section>}
    {step === 5 && <section className="space-y-5"><div><h3 className="text-lg font-black tracking-[-.03em] text-[#18202b]">첫 글 주제와 입력 내용을 확인해 주세요.</h3><p className="mt-1.5 text-xs leading-5 text-[#7b838b]">저장 후 새 초안 화면에서 주제와 키워드만 선택해 초안을 만들 수 있습니다.</p></div><div className="space-y-4"><div><label htmlFor="onboarding-first-topic" className={labelClass}>첫 글 주제</label><textarea id="onboarding-first-topic" value={form.firstTopic} onChange={event => update("firstTopic", event.target.value)} placeholder="예: 합정동 필라테스 초보자가 첫 수업 전에 준비할 것" className={textareaClass}/></div><div className="rounded-xl border border-[#e8e3dc] bg-white p-4"><p className="text-xs font-extrabold text-[#18202b]">생성 방식</p><p className="mt-1.5 text-xs leading-5 text-[#7b838b]">기본 정보만으로 시작하고, 이후 기존 글 URL·공식 홈페이지·사진을 추가해 글의 정확도를 높일 수 있습니다.</p></div></div></section>}
